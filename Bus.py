...
#START GLOBAL DATA
########################################################################################################################
curLat = 39.9509507 #once get GPS data, write to curLat and curLon 
curLon =  -75.185327
phiD = 0 #send to Arduino with velocity
desiredOffset = 3
curOffset = 3
camLookLeft = True
speed = 20
...
##  get Arduino code by going through PiCommands
    for piCommand in range(4){ ## go through each of the GPS variables sent
        byteList = [phiD, speed]
        if sendMode = True {#either send or receive from Arduino
            I2C.putByteList(piCommand, byteList) ## sends piCommand and byteList = [phiD, speed]
            }
        if sendMode = False{
            data = I2C.getByteData() ## gets GPS data from arduino
            storeValues[piCommand] = bytes_2_float(data) ## convert bytes to float and store in array
        }
    }
    parse_serial(storeValues)## writes GPS to text file
...
##    speedCurrent = I2C.getSpeed() ##not sure how sending speed --> need to decide as lab group whether using PWM
##
